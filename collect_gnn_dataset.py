...
def _canonical_url_key(url: str) -> str:
    try:
        graph = build_web_graph(url, fetch=False)
        return graph.page_url.lower().rstrip("/")
    except Exception:
...
def _collect_row(row: Dict[str, str]) -> Dict[str, str]:
    url = (row.get("url") or "").strip()
    if not url:
        return row
    out = dict(row)
    graph = build_web_graph(url, fetch=True)
...
def _collect_many(rows: List[Dict[str, str]], workers: int) -> List[Dict[str, str]]:
    collected: List[Optional[Dict[str, str]]] = [None] * len(rows)
    with ThreadPoolExecutor(max_workers=max(1, workers)) as pool:
        future_map = {pool.submit(_collect_row, row): i for i, row in enumerate(rows)}
...
def main() -> int:
    ...
    if args.input:
        ...
        targets.extend(_load_input_rows(input_path, args.source))
    ...
    collected = _collect_many(targets, args.workers)
